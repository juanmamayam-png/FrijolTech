...
describe('POST /api/v1/auth/register', () => {
  test('registra un usuario nuevo y retorna 201', async () => {
    ...
    const res = await request(app)
      ...
      .send({ nombre: 'Juan Agricultor', correo: 'juan@test.com', contraseña: 'password123' });
  ...
  test('retorna 409 si el correo ya existe', async () => {
    ...
    const res = await request(app)
      .post('/api/v1/auth/register')
      .send({ nombre: 'Juan', correo: 'juan@test.com', contraseña: 'password123' });
  ...
  test('retorna 422 si falta el campo correo', async () => {
    const res = await request(app)
      .post('/api/v1/auth/register')
      .send({ nombre: 'Juan', contraseña: 'password123' });
...
describe('POST /api/v1/auth/login', () => {
  test('retorna 200 con JWT para credenciales válidas', async () => {
    ...
    const res = await request(app)
      ...
      .send({ correo: 'maria@test.com', contraseña: 'mipassword' });
  ...
  test('retorna 401 para contraseña incorrecta', async () => {
    ...
    mockUsuarioBuscarPorCorreo.mockResolvedValue({
      id: 2, correo: 'maria@test.com', contraseñaHash: hash, rolId: 2, nombre: 'María',
    });

    const res = await request(app)
      .post('/api/v1/auth/login')
      .send({ correo: 'maria@test.com', contraseña: 'incorrecta' });
  ...
  test('retorna 401 si el usuario no existe', async () => {
    ...
    const res = await request(app)
      .post('/api/v1/auth/login')
      .send({ correo: 'noexiste@test.com', contraseña: 'password123' });
