...
test('Factory Method: FabricaCargamanto genera 7 etapas con ciclo de ~110 días', () => {
  ...
  const cronograma = fabrica.generarCronograma(CAMPAÑA_ID, FECHA_SIEMBRA);
...
test('Factory Method: FabricaICACerinza genera umbrales de temperatura menores (clima frío)', () => {
  ...
  const cronogramaICA = fabricaICA.generarCronograma(CAMPAÑA_ID, FECHA_SIEMBRA);
  const cronogramaCargamanto = fabricaCargamanto.generarCronograma(CAMPAÑA_ID, FECHA_SIEMBRA);
