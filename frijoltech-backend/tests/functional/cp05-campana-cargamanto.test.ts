/**
 * CP-05: Iniciar campaña Cargamanto (RF-05 — Camino feliz + Factory Method)
...
beforeAll(() => {
  mockCampañaCrear.mockResolvedValue({
    id: 1, ...DATOS_PRUEBA.campañaCargamanto, estado: 'activa', createdAt: new Date().toISOString(),
  });
...
test('CP-05: Campaña Cargamanto genera exactamente 7 etapas fenológicas', async () => {
  const inicio = Date.now();
  const res = await request(app)
    .post('/api/v1/campa%C3%B1as')
    .set('Authorization', `Bearer ${token}`)
    .send(DATOS_PRUEBA.campañaCargamanto);
  tiempoMs = Date.now() - inicio;

  expect(res.status).toBe(201);
  expect(res.body.data.campaña).toHaveProperty('id');
...
afterAll(() => {
  guardarResultado({
    id: 'CP-05', nombre: 'Campaña Cargamanto — Factory Method', rf: 'RF-05', tipo: 'Camino feliz + Factory',
