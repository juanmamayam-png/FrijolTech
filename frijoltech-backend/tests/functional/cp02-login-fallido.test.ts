...
test('CP-02a: Contraseña incorrecta retorna 401', async () => {
  ...
  const res = await request(app)
    ...
    .send({ correo: DATOS_PRUEBA.usuario.correo, contrasena: 'contraseña_INCORRECTA' });
...
test('CP-02b: Usuario inexistente retorna 401', async () => {
  ...
  const res = await request(app)
    .post('/api/v1/auth/login')
    .send({ correo: 'noexiste@test.com', contrasena: 'cualquier_cosa' });
...
test('CP-02c: Correo con formato inválido retorna 422', async () => {
  const res = await request(app)
    .post('/api/v1/auth/login')
    .send({ correo: 'no-es-un-email', contrasena: 'password' });
