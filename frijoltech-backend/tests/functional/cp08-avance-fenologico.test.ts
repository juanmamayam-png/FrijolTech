 ...
 * NOTA: El tiempo puede ser > 1s por el procesamiento de imagen en el cliente.
 */
...
beforeAll(() => {
  mockEtapaBuscar.mockResolvedValue({
    id: ETAPA_ID, nombre: 'Floración', orden: 4, duracionDias: 15,
    umbralTempMin: 15, umbralTempMax: 26, umbralHumedadMin: 70,
    fechaEstimada: '2026-05-07', estado: 'en_curso', campañaId: 1,
  });
...
test('CP-08: Avance sin foto → 201 con registroId', async () => {
  ...
  const res = await request(app)
    ...
    .send({
      campañaId: 1,
      observaciones: 'Floración uniforme en el 90% del área. Pétalos blancos vigorosos. Sin síntomas de antracnosis hasta el momento. Temperatura media 22°C.',
    });
...
test('CP-08b: Avance con fotoUrl válida → 201', async () => {
  ...
  const res = await request(app)
    ...
    .send({
      campañaId: 1,
      observaciones: 'Avance con foto adjunta. Floración en etapa media.',
  ...
  if (elapsed > 2000) {
    ...
    observaciones = `Tiempo ${elapsed}ms excede 2s. Considerar procesamiento asíncrono de imágenes grandes.`;
  }
  expect(res.status).toBe(201);
});
...
test('CP-08c: Observaciones < 5 caracteres → 422', async () => {
  const res = await request(app)
    .post(`/api/v1/etapas/${ETAPA_ID}/avance`)
    .set('Authorization', `Bearer ${token}`)
    .send({ campañaId: 1, observaciones: 'ok' });

  expect(res.status).toBe(422);
  if (!observaciones) observaciones = `registroId=${77} persistido. Validación de observaciones mínimas correcta.`;
});

afterAll(() => {
  guardarResultado({
    id: 'CP-08', nombre: 'Registrar avance fenológico', rf: 'RF-07', tipo: 'Camino feliz',
