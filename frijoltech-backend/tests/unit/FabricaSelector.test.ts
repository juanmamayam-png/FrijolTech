...
describe('FabricaSelector (Factory Method)', () => {
  ...
  test('Cargamanto genera 7 etapas con temperaturas 15-28°C', () => {
    const fabrica = FabricaSelector.obtenerFabrica('Cargamanto');
    const etapas = fabrica.generarCronograma(CAMPANA_ID, FECHA_SIEMBRA);
  ...
  test('ICA Cerinza genera 7 etapas con temperaturas más bajas que Cargamanto', () => {
    ...
    const etapasCargamanto = cargamanto.generarCronograma(CAMPANA_ID, FECHA_SIEMBRA);
    const etapasCerinza = cerinza.generarCronograma(CAMPANA_ID, FECHA_SIEMBRA);
  ...
  test('ICA Cerinza tiene ciclo más largo que Cargamanto', () => {
    ...
    const diasCargamanto = cargamanto.generarCronograma(CAMPANA_ID, FECHA_SIEMBRA)
      .reduce((acc, e) => acc + e.duracionDias, 0);
    const diasCerinza = cerinza.generarCronograma(CAMPANA_ID, FECHA_SIEMBRA)
      .reduce((acc, e) => acc + e.duracionDias, 0);
  ...
  test('las 3 variedades producen fechas estimadas distintas para la etapa de floración', () => {
    ...
    const fechasOrden4 = variedades.map((v) => {
      const etapas = FabricaSelector.obtenerFabrica(v).generarCronograma(CAMPANA_ID, FECHA_SIEMBRA);
