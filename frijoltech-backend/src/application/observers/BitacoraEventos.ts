...
export class BitacoraEventos implements IObservador {
  ...
  async actualizar(evento: EventoCritico): Promise<void> {
    try {
      await this.db.query(
        `INSERT INTO bitacora_eventos (tipo_evento, valor_observado, umbral_esperado, fecha, campaña_id, etapa_actual)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [evento.tipo, evento.valorObservado, evento.umbralEsperado, evento.fecha, evento.campañaId, evento.etapaActual]
      );
      console.log(`[BitacoraEventos] Evento ${evento.tipo} persistido para campaña ${evento.campañaId}`);
    } catch (error) {
