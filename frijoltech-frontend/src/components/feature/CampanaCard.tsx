...
const estadoBadge: Record<Campana['estado'], { label: string; variant: 'green' | 'yellow' | 'gray' }> = {
...
export function CampanaCard({ campana, onClick }: CampanaCardProps) {
  const badge = estadoBadge[campana.estado];
  return (
    <Card
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="font-bold text-dark font-serif truncate">Lote #{campana.loteId}</span>
            <Badge variant={badge.variant}>{badge.label}</Badge>
          </div>
          <div className="flex items-center gap-1 text-muted text-sm">
            <Calendar className="w-3.5 h-3.5" />
            <span>Siembra: {formatDateShort(campana.fechaSiembra)}</span>
          </div>
          <p className="text-sm text-muted mt-1">{campana.areaSembrada} ha sembradas</p>
        </div>
