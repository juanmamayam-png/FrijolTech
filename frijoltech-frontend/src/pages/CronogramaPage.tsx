...
export function CronogramaPage() {
  ...
  const { campaña: data, loading, error } = useCampaña(id ? Number(id) : null);

  function handleRegistrarAvance(etapa: EtapaFenologica) {
    navigate(`/etapas/${etapa.id}/avance`, {
      state: { etapaNombre: etapa.nombre, campañaId: data?.campaña.id },
    });
  }
      ...
      <Container>
        ...
        <div className="bg-primary rounded-2xl p-5 mb-6 text-white">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-bold font-serif">Lote #{campaña.loteId}</span>
            <Badge variant="green">{campaña.estado}</Badge>
          </div>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <p className="font-semibold flex items-center gap-1 mt-0.5">
                ...
                {formatDateShort(campaña.fechaSiembra)}
              </p>
            </div>
            <div>
              <p className="text-secondary-200 text-xs">Área sembrada</p>
              <p className="font-semibold mt-0.5">{campaña.areaSembrada} ha</p>
            </div>
          </div>
          {etapaEnCurso && (
